...
##取得会员最早订单权益函数,可缓存数据
def __account_regist(jdbc_df_1,date):
    """取得会员最早订单的一条数据"""
...
(select t1.fsk_account_name,t1.fsk_order_type,t1.fsk_package_provide,t1.fsk_package_id,t1.fsk_right_starttime,t1.fsk_right_limittime,t1.fsk_order_addtime,min(t1.fsk_order_addtime) over(partition by fsk_account_name) registtime
...
def __account_new_report(report_0,report_1,report_7):
    """新增会员权益包信息报表 """
    ##天环比、周同比连接条件
...
def main():
    """ account_new_report_v2"""
    #boss拉取数据
    jdbc_df_1 = __boss_to_hdfs()
    ##缓存
    jdbc_df_1.persist(pyspark.StorageLevel.MEMORY_AND_DISK)

    jdbc_df_2_0 = __account_regist(jdbc_df_1=jdbc_df_1,date=__str_dt_0)
    jdbc_df_2_1 = __account_regist(jdbc_df_1=jdbc_df_1,date=__str_dt_1)
    jdbc_df_2_7 = __account_regist(jdbc_df_1=jdbc_df_1,date=__str_dt_7)

    ##新增会员数据
    report_0 = __account_new(jdbc_df_2=jdbc_df_2_0)
    report_1 = __account_new(jdbc_df_2=jdbc_df_2_1)
    report_7 = __account_new(jdbc_df_2=jdbc_df_2_7)
    ##新增会员报表数据
    report =  __account_new_report(report_0=report_0,report_1=report_1,report_7=report_7)
    ##报表数据写入MySQL
    __hdfs_to_mysql(src_df=report,table_name="account_new_report_v2",mode_type="append")
